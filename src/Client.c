...
void *receive(void *sockfd)
{
    ...
    {
        if(strncmp(recv, "#STARTW#", sizeof("#STARTW#")) == 0) // initialize board
        {
            printf("You have started chess. You are the white player.\n");
            BOARD *board = chess();
            for(int i = 0; i < 8; i++)
            {
                for(int j = 0; j < 8; j++)
        ...
        else if(strncmp(recv, "#STARTB#", sizeof("#STARTB#")) == 0) // initialize board
        {
            printf("Your friend has started chess. You are the black player.\n");
            BOARD *board = chess();
            for(int i = 0; i < 8; i++)
            {
                for(int j = 0; j < 8; j++)
        ...
        if(strncmp(recv, "#STARTW#", sizeof("#STARTW#")) != 0)
        {
            if(strncmp(recv, "#STARTB#", sizeof("#STARTB#")) != 0)
            {
                recv[strlen(recv)] = '\0';
                fputs(recv, stdout);
            }
