...
void addFriend(GtkWidget *widget, gpointer data){
    ...
    if(add == 0)
    {
        ...
        GtkWidget *errfrnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
        GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(errfrnd), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                   GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                   "This user is already your friend, or they do not exist.");
        gtk_window_set_position(GTK_WINDOW(errfrnd), GTK_WIN_POS_CENTER) ;
...
void append_item(GtkWidget *widget, gpointer entry) {
    ...
    if (*str != '\0'){
        ...
        GtkWidget *err_search = gtk_window_new(GTK_WINDOW_TOPLEVEL);
        GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(err_search), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                   GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                   "Search cannot be empty! Please try again.");
        gtk_window_set_position(GTK_WINDOW(err_search), GTK_WIN_POS_CENTER) ;
        gtk_window_set_title(GTK_WINDOW(dialog), "Warning");
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
    }
...
void chatting(GtkWidget *widget, gpointer data){
    ...
    GtkTreeIter  iter;
    gchar *chatFriend;
    char *init = malloc(sizeof(char)*1007);
    ...
    if (gtk_tree_selection_get_selected(GTK_TREE_SELECTION(sendData->selection),
                                        &model, &iter)) {
        gtk_tree_model_get(model, &iter, LIST_ITEM, &chatFriend, -1);
        char *chat = (char *)chatFriend;
        strcpy(init, "#CHAT#\n");
        strcat(init, chat);
        u_int32_t userlen = htonl(strlen(init));
        write(sendData->sockfd, &userlen, sizeof(userlen));
        write(sendData->sockfd, init, strlen(init));
...
void ChatMainMenu(char username[MAX_LENGTH], int sockfd) {
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Chesster Chat");
    ...
    gtk_widget_set_size_request(addBtn, 70, 30);
