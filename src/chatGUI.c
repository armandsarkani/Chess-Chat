...
void screen_Login(GtkWidget *widget, gpointer data){
    ...
    if(registerOpen == 0)
    {
        ...
        {
            blank = 1;
            GtkWidget *errUE = gtk_window_new(GTK_WINDOW_TOPLEVEL);
            GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(errUE), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                       GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                       "Username cannot be empty!");
            gtk_window_set_position(GTK_WINDOW(errUE), GTK_WIN_POS_CENTER) ;
        ...
        {
            blank = 1;
            GtkWidget *errPE = gtk_window_new(GTK_WINDOW_TOPLEVEL);
            GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(errPE), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                       GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                       "Password cannot be empty!");
            gtk_window_set_position(GTK_WINDOW(errPE), GTK_WIN_POS_CENTER) ;
            gtk_window_set_title(GTK_WINDOW(dialog), "Warning");
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
        }
        ...
        if(blank == 0)
        {
            ...
            {
                GtkWidget *errU = gtk_window_new(GTK_WINDOW_TOPLEVEL);
                GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(errU), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                           GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                           "The password you entered is incorrect. Please try again.");
                gtk_window_set_position(GTK_WINDOW(errU), GTK_WIN_POS_CENTER) ;
                gtk_window_set_title(GTK_WINDOW(dialog), "Warning");
                gtk_dialog_run(GTK_DIALOG(dialog));
                gtk_widget_destroy(dialog);
            }
            ...
            {
                GtkWidget *err = gtk_window_new(GTK_WINDOW_TOPLEVEL);
                GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(err), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                           GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
...
void Register_function(GtkWidget *widget, gpointer data){ //When Register Button is clicked
    ...
    if(*entry_CPW_Text == '\0')
    {
        GtkWidget *errCPE = gtk_window_new(GTK_WINDOW_TOPLEVEL);
        GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(errCPE), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                   GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                   "Confirm password cannot be empty!");
        gtk_window_set_position(GTK_WINDOW(errCPE), GTK_WIN_POS_CENTER) ;
        gtk_window_set_title(GTK_WINDOW(dialog), "Warning");
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
        blank = 1;
    ...
    if(blank == 0)
    {
        if(ComparePasswords(entry_PW_Text, entry_CPW_Text) == 0)
        {
            ...
            if(pass == 1)
            {
                GtkWidget *err_taken = gtk_window_new(GTK_WINDOW_TOPLEVEL);
                GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(err_taken), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                           GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                           "The username you entered has already been taken. Please try again.");
                gtk_window_set_position(GTK_WINDOW(err_taken), GTK_WIN_POS_CENTER) ;
                gtk_window_set_title(GTK_WINDOW(dialog), "Warning");
                gtk_dialog_run(GTK_DIALOG(dialog));
                gtk_widget_destroy(dialog);
                //g_print("pass = %d\n", pass);
            ...
            else if(pass == 0)
            {
                ...
                GtkWidget *success = gtk_window_new(GTK_WINDOW_TOPLEVEL);
                GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(success), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                           GTK_MESSAGE_OTHER, GTK_BUTTONS_OK,
                                                           "Successfully registered!");
                gtk_window_set_position(GTK_WINDOW(success), GTK_WIN_POS_CENTER) ;
        ...
        {
            GtkWidget *err_match = gtk_window_new(GTK_WINDOW_TOPLEVEL);
            GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(err_match), GTK_DIALOG_DESTROY_WITH_PARENT,
                                                       GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                       "Passwords do not match! Please try again.");
            gtk_window_set_position(GTK_WINDOW(err_match), GTK_WIN_POS_CENTER) ;
            gtk_window_set_title(GTK_WINDOW(dialog), "Warning");
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
            //g_print("Contents of entries:\n%s\n%s\n%s\n", entry_Username_Text, entry_PW_Text, entry_CPW_Text);
...
void CreateRegisterMenu(REGISTERDATA *registerData){
    ...
    g_signal_connect(registerData->entry_Username, "activate", G_CALLBACK(Register_function), (gpointer)registerData);
    g_signal_connect(registerData->entry_PW, "activate", G_CALLBACK(Register_function), (gpointer)registerData);
    g_signal_connect(registerData->entry_CPW, "activate", G_CALLBACK(Register_function), (gpointer)registerData);
...
void CreateLoginMenu(MENUDATA *menuData){
    ...
    g_signal_connect(menuData->entry_Login, "activate", G_CALLBACK (screen_Login), (gpointer)menuData);
    g_signal_connect(menuData->entry_PW, "activate", G_CALLBACK (screen_Login), (gpointer)menuData);
